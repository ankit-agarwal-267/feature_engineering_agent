...
class InteractionTransformer:
    ...
    def generate_numeric_interactions(self, df: RawDataFrame, pairs: List[Tuple[str, str]]) -> RawDataFrame:
        ...
        if isinstance(df, pd.DataFrame):
            ...
            for col_a, col_b in pairs:
                res[f"{col_a}_x_{col_b}"] = df[col_a] * df[col_b]
            return res.astype('float32')
        else:
            new_cols = []
            for col_a, col_b in pairs:
                new_cols.append((df[col_a] * df[col_b]).alias(f"{col_a}_x_{col_b}").cast(pl.Float32))
    ...
    def generate_categorical_interactions(self, df: RawDataFrame, pairs: List[Tuple[str, str]]) -> RawDataFrame:
        """
        Section 7.7.2: Concatenate string values of two low-cardinality columns.
        """
        if isinstance(df, pd.DataFrame):
            res = pd.DataFrame(index=df.index)
            for col_a, col_b in pairs:
                res[f"{col_a}__x__{col_b}"] = df[col_a].astype(str) + "__" + df[col_b].astype(str)
            return res
        else:
            new_cols = []
            for col_a, col_b in pairs:
                new_cols.append((df[col_a].cast(pl.String) + "__" + df[col_b].cast(pl.String)).alias(f"{col_a}__x__{col_b}"))
            return pl.DataFrame(new_cols)
